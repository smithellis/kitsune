...
class TermToken(BaseToken):
    ...
    def elastic_query(self, context):
        ...
        query_params = {
            "query": self.term,
            "default_operator": "AND" if len(terms) <= 3 else "OR",  # Stricter for short queries
            "fields": context["fields"],
        }

        # Add stricter minimum_should_match for multi-term queries
        if query_params["default_operator"] == "OR" and len(terms) >= 2:
            if len(terms) == 2:
                query_params["minimum_should_match"] = "100%"  # Both must match
            elif len(terms) == 3:
