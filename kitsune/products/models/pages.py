...
class ProductPagePassThrough(Page):
    """A page that passes through to a Product"""

    products = Product.objects.filter(visible=True)

    choices = [(product.slug, product.title) for product in products]
    product = blocks.ChoiceBlock(choices=choices)

    def serve(request, *args, **kwargs):
        return product_landing(request, kwargs["product"])
